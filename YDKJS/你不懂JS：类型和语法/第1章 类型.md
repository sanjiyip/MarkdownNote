...
typeof的安全防范机制格式：
```js
if(typeof variable === "function") {    //检查的全局变量variable，是否已被声明
    /* ..... */
}
```
...
还有一种不通过typeof的安全防范机制的方法，就是**检查所有全局变量是否是全局对象的属性，浏览器中的全局对象是window**。
...
与undeclared变量不同，访问不存在的对象属性不会产生ReferenceError错误。（因为对象属性可以自动被添加（对象没有的前提下））。——原型部分的内容。
